...
static void init_array(int ni, int nj, int nk, int nl, double * alpha, double * beta, double A[(2000+0)][(2000+0)], double B[(2000+0)][(2000+0)], double C[(2000+0)][(2000+0)], double D[(2000+0)][(2000+0)])
{
	int i, j;
	( * alpha)=32412;
	( * beta)=2123;
...
*/
static void print_array(int ni, int nl, double D[(2000+0)][(2000+0)])
{
	int i, j;
	for (i=0; i<ni; i ++ )
	{
...
*/
/*
rcv buffer for accessed data - D
*/
double rcvBuffD[(2000+0)][(2000+0)];
int main(int argc, char * * argv)
{
	...
	*/
	...
	#pragma MPI  accessed_data(tmp[i][j])
	for (i=0; i<2000; i ++ )
	/*
	...
	*/
	if (size>1)
	{
		...
		{
			/*
			For accessed data - tmp - all-to-all broadcasting
			*/
			{
				Ld=0;
				Ud=2000;
				...
				MPI_Bcast(&tmp[CD_L_p][0], CD_S_p, MPI_DOUBLE, p, MPI_COMM_WORLD);
			}
		}
		mpipar_comm_timer_stop();
	}
	#pragma MPI  parallel 0
	#pragma MPI  accessed_data(D[i][j])
	for (i=0; i<2000; i ++ )
	/*
	Computation partitioning and mapping code begins 
	*/
		...
		*/
		{
			...
			{
				CD_L_p=(CD_L_p+Rd);
				memcpy(rcvBuffD, D, sizeof(D));
			}
			CD_S_p=((CD_U_p-CD_L_p)*PSM);
			MPI_Gather(&D[CD_L_p][0], CD_S_p, MPI_DOUBLE, &rcvBuffD[CD_L_p][0], CD_S_p, MPI_DOUBLE, 0, MPI_COMM_WORLD);
			if (rank==0)
			{
				memcpy(D, rcvBuffD, sizeof(rcvBuffD));
			}
		}
		mpipar_comm_timer_stop();
	}
