...
static void init_array(int nx, int ny, double A[(8000+0)][(8000+0)], double r[(8000+0)], double p[(8000+0)])
{
	int i, j;
	for (i=0; i<ny; i ++ )
	{
		p[i]=(i*3.141592653589793);
	}
	for (i=0; i<nx; i ++ )
	{
		r[i]=(i*3.141592653589793);
		for (j=0; j<ny; j ++ )
		{
...
*/
static void print_array(int nx, int ny, double s[(8000+0)], double q[(8000+0)])
{
	int i;
	for (i=0; i<ny; i ++ )
	{
...
/*
rcv buffer for accessed data - s
*/
double rcvBuffs[(8000+0)];
	...
	*/
		...
		*/
		{
			...
			{
				CD_L_p=(CD_L_p+Rd);
				memcpy(rcvBuffs, s, sizeof(s));
			}
			CD_S_p=((CD_U_p-CD_L_p)*PSM);
			MPI_Gather(&s[CD_L_p], CD_S_p, MPI_DOUBLE, &rcvBuffs[CD_L_p], CD_S_p, MPI_DOUBLE, 0, MPI_COMM_WORLD);
			if (rank==0)
			{
				memcpy(s, rcvBuffs, sizeof(rcvBuffs));
			}
		}
	...
	#pragma omp parallel private(i, j)
	{
		...
		#pragma omp critical
		{
			for (reduce_span_0=0; reduce_span_0<8000; reduce_span_0 ++ )
			{
				s[reduce_span_0]+=reduce[reduce_span_0];
			}
		}
	}
