...
#run parallel program each five times and take the average
			totalPar4=0;
			for i in 1 2 3 4 5;do
				#for node=4
				parallel_extime_n4=$(mpiexec -f machinefile -n 4 $outputDir/$benchname);
				totalPar4=$(echo $totalPar4 + $parallel_extime_n4 |bc);
			done
			parallel_extime_n4_avg=$(echo "scale=4;$totalPar4 / 5" |bc);
			#dump the output in $outputDir/$benchname.out file
	                echo $parallel_extime_n4_avg > $outputDir/$benchname'.tm'
